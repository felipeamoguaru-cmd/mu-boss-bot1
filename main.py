...
CARGO_AVISO = "CaçadorDeBoss"  # ← Nome exato do cargo no seu servidor (melhoria 3)
PAINEL_MSG_ID = None  # Será preenchido automaticamente (melhoria 5)
# ============================================================
...
# ─── BOSSES FIXOS (melhoria 2 — mais bosses adicionados) ───
# Bosses com horários FIXOS
BOSSES_FIXOS = {
    "Death Bone": {
        "local": "Shadow Abyss",
...
# Bosses com respawn por TEMPO após a morte
BOSSES_TIMER = {
...
def proximo_respawn_fixo(boss_nome):
    ...
    horarios = BOSSES_FIXOS[boss_nome]["horarios"]
    proximos = []
    for h in horarios:
...
def get_cargo_mention(guild):
    """Retorna a menção ao cargo ou @here se não achar."""
...
def build_painel_embed():
    """Monta o embed do painel ao vivo."""
    agora = datetime.now(tz)
    embed = discord.Embed(
...
async def criar_painel(ctx):
    """!painel — Cria/recria a mensagem de painel ao vivo."""
...
async def boss_morreu(ctx, *, nome_boss: str = None):
    ...
    agora = datetime.now(tz)
    dt_respawn = agora + timedelta(hours=info["horas_min"])

    dados = carregar_dados()
    dados[boss_encontrado] = dt_respawn.isoformat()
    salvar_dados(dados)

    embed = discord.Embed(
        title=f"{info['emoji']} {boss_encontrado} foi derrotado!",
    ...
    embed.add_field(name="⏱️ Tempo", value=f"~{int(info['horas_min'])}h - {int(info['horas_max'])}h", inline=True)
...
async def resetar_boss(ctx, *, nome_boss: str = None):
    """!resetar <nome> — Remove o timer de um boss registrado errado."""
    if nome_boss is None:
...
async def ajuda(ctx):
    ...
    embed.add_field(name="`!morreu <nome>`", value="Registrar morte de um boss de timer", inline=False)
...
# ─────────────────────────────────────────────
#  TAREFA — Avisos automáticos (melhoria 3: cargo)
# ─────────────────────────────────────────────
...
# ─── MELHORIA 5: Painel que se atualiza sozinho ───
@tasks.loop(seconds=30)
